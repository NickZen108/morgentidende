...
begin
 ...
 if direct_kind='direct_article_v2' then
  if not exists (
   select 1 from public.v3_orders o
   join public.v3_chat_receipts r on o.dedupe_key='chatops:direct-v2:'||r.id::text
   where o.id=new.order_id
     and r.workflow_id='chatops-publish-'||r.id::text
     and r.status in ('accepted','dispatched')
     and r.payload_hash ~ '^[a-f0-9]{64}
 end if;
