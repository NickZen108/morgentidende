...
export function orderResult(order:StatusOrder,attempt:Attempt|undefined,article:Published|undefined,origin:string){
 ...
 reason:article?null:order.error_code==='daily_budget_exhausted'?'Dagsbudgettet kan ikke dække næste kald. Ordren kræver genoptagelse.':attempt?.review?.reason??(order.status==='failed'?'Produktionen stoppede med en teknisk fejl.':null),
