 ...
 try{
  const response=await (env.AI.run.bind(env.AI) as Run)('@cf/google/gemma-4-26b-a4b-it',{
   messages:[{role:'user',content:[
    {type:'text',text:`Judge whether this image is suitable as a contextual editorial illustration for this article. It does not need to depict the actual event. Reject images that are misleading, irrelevant or unreadable. Answer with exactly SUITABLE or REJECT. Article: ${article.headline}. ${article.deck}`},
...
export async function selectMedia(env:Env,article:Draft,job:string):Promise<MediaRow>{
 ...
 url.search=new URLSearchParams({
  action:'query',format:'json',generator:'search',gsrsearch:article.image_query,gsrnamespace:'6',gsrlimit:'5',
  prop:'imageinfo',iiprop:'url|sha1|extmetadata',iiurlwidth:'1600'
 }).toString();
 try{
  ...
  for(const page of Object.values(data.query?.pages??{})){
   ...
   const [asset]=await db<MediaRow[]>(env,'v3_media?on_conflict=original_url','POST',{
    ...
    credit:plain(info.extmetadata.Artist?.value??'Wikimedia Commons'),
    alt:plain(info.extmetadata.ImageDescription?.value??article.image_query).slice(0,600),
   ...
   try{
    const identified=await registerIdentity(env,asset);
    if(await identityEligible(env,identified))return identified;
   }catch{continue;}
  }
 }catch(error){
  console.log(JSON.stringify({event:'commons_failed',reason:error instanceof Error?error.message:'unknown'}));
 }
 ...
 for(let variant=0;variant<prompts.length;variant++){
  ...
  if(!object){
   ...
   if(!result.image){
    console.log(JSON.stringify({event:'flux_image_missing',variant:variant+1}));
    continue;
   }
  ...
  if(visionResult===false){
   console.log(JSON.stringify({event:'generated_image_rejected',variant:variant+1}));
   continue;
  }
  ...
  await ensureFamily(env,family);
  const [asset]=await db<MediaRow[]>(env,'v3_media?on_conflict=original_url','POST',{
   family_id:family,content_hash:hash,original_url:publicUrl,url:publicUrl,
   credit:'AI-illustration · Morgentidende / FLUX',alt:`Illustration: ${article.image_query}`,
   license_documentation:{
    license:'FLUX.1-schnell Apache-2.0',
    license_url:'https://huggingface.co/black-forest-labs/FLUX.1-schnell/blob/main/LICENSE',
    evidence:{provider:'Cloudflare Workers AI',model:'@cf/black-forest-labs/flux-1-schnell',prompt,object:key,variant:variant+1},
  ...
  try{
   ...
   console.log(JSON.stringify({event:'generated_identity_failed',variant:variant+1,reason:error instanceof Error?error.message:'unknown'}));
  }
 }
