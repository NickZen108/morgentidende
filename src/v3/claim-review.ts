...
export async function reviewWithEvidence(env:Env,step:WorkflowStep,input:Input){
 ...
 const assess=async(label:string)=>{
  const result=await step.do(`${prefix}-evidence-assess-${label}`,CHECK_STEP,async()=>{
   ...
   return model(env,'chief',instructions,{original_order:input.original_order,article,state,previous_checks:review?.claim_checks??[],snapshots:snapshots.map(s=>({...s,text:s.text.slice(0,3600)}))},Assessment,false,'claim-review');
  });
  // Validate against exactly the text supplied to the reviewer, not unseen source content.
