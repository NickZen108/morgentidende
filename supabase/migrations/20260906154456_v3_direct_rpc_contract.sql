CREATE OR REPLACE FUNCTION public.v3_publish_direct(p_order uuid, p_media uuid, p_slot text, p_article jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
begin
 ...
 if not exists(select 1 from public.v3_orders where id=p_order and original_order->>'kind'='direct_article_v2') then
  raise exception 'direct_order_required';
 end if;
 if not exists(
  select 1 from public.v3_orders o join public.v3_chat_receipts r on o.dedupe_key='chatops:direct-v2:'||r.id::text
  where o.id=p_order and r.workflow_id='chatops-publish-'||r.id::text
   and r.status in ('accepted','dispatched') and r.payload_hash ~ '^[a-f0-9]{64}$'
 ) then raise exception 'direct_chat_receipt_required'; end if;
