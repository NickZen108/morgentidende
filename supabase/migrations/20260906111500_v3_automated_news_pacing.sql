-- Enable the automated newsroom after the Indland/Udland-only admission rule is in place.
-- Add pacing context so Chief can spread strong stories over the day without a hard time gate.
...
grant execute on function public.v3_editorial_state() to service_role;
