...
begin
 ...
 update public.v3_attempts set review=review||'{"serious_error":false,"evidence_status":"verified","verification_version":1,"claim_checks":[{"status":"supported","source_verified":true}]}'::jsonb where order_id=o;
 update public.v3_attempts set review=review||'{"evidence_status":"unresolved"}'::jsonb where order_id=o;
 begin
  perform public.v3_publish(o,1,'lead');
  raise exception 'unresolved_claim_published';
 exception when others then
  if sqlerrm <> 'claim_evidence_unresolved' then raise; end if;
 end;
 update public.v3_attempts set review=review||'{"evidence_status":"verified"}'::jsonb where order_id=o;
